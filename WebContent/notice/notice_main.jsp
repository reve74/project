...
<body>
  ...
  <script>
	...
	function fnList() {
		...
		document.list.action = "notice_main.jsp";
...
<%
	...
	Vector v = ndao.getBoardList(keyField, keyWord);
...
<% 
									...
									for(int cnt=beginPerPage; cnt<(beginPerPage+numPerPage); cnt++) {
										...
										nboardDTO ndto = (nboardDTO)v.get(cnt);
					%>
							
								<tr>
									<td><%=ndto.getNum()%></td>
						 			<td class="tab_sub">	
						 			<a href="#" onclick="fnRead('<%=ndto.getNum()%>'); return false;"><%=ndto.getSubject()%></a></td>
									<td><%=ndto.getId()%></td>
									<td><%=s.format(ndto.getDate())%></td>
...
<%	
				if(totalBlock > 0){
					...
					if(nowBlock > 0){
%>			
				<a href="notice_main.jsp?nowBlock=<%=nowBlock-1%>&nowPage=<%=(nowBlock-1)*pagePerBlock%>&keyWord=<%=keyWord%>&keyField=<%=keyField%>">
					&laquo;
					</a>
<%			
					}
...
%>		
					<%--페이지 번호 링크  --%>
					<a href="notice_main.jsp?nowBlock=<%=nowBlock%>&nowPage=<%=(nowBlock*pagePerBlock)+cnt%>&keyWord=<%=keyWord%>&keyField=<%=keyField%>">			
...
%>
					...
					<div id="btnarea">
						<div style="margin:0 auto; min-width:800px; text-align: center;">
...
%>							
								...
								<button type="button" class="btn wrbtn" id="btn1"
									onclick="location.href='noticewrite.jsp'">Write</button>
									&nbsp;&nbsp;
	 ...
	 <form action="notice_read.jsp" method="post" name="read">
